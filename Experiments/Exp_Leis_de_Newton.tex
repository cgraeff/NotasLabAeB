...
Podemos calcular a aceleração a partir de um valor qualquer de deslocamento e do tempo correspondente ao invertermos a relação acima:
\begin{equation}\label{LeisDeNewton:Eq:AceleracaoCinematica}
	a = \frac{2 \Delta x}{t^2}.
\end{equation}
%
Para podermos comparar o valor da aceleração obtida através da cinemática, podemos determinar a aceleração para cada um dos deslocamentos e então fazer uma média dos valores obtidos, o que resultará em uma eliminação dos erros aleatórios associados à determinação dos valores da variável dependente. Veremos nos próximos experimentos um método melhor para determinar um parâmetro a partir de um conjunto de dados, uma vez que uma média simples está suscetível a desvios devido a pontos discrepantes e também corre o risco de amplificar ruídos nos valores do conjunto de dados.\footnote{A eliminação dos erros depende de uma dispersão aleatória das medidas em relação a um valor mais provável e a consequente eliminação devido a diferenças de sinal. No caso de calcularmos um parâmetro que envolva o quadrado das variáveis independente e dependente, por exemplo, essa eliminação não ocorre.}
...
\begin{itemize}
     ...
     \item Calcular a aceleração do sistema através do valor médio de diversas medidas;
...
\begin{enumerate}
    ...
    \item Posicione o carrinho em contato com o eletroímã e utilize o fio para suspender o gancho com as anilhas, passando pela polia.
	\item Posicione o sensor a uma distância de cerca de \np[cm]{8,00} do pino central do carrinho.
	\item Pressione \texttt{START} para liberar o carrinho para que se mova e anote os valores obtidos de tempo e de distância percorrida na Tabela~\ref{TabelaDadosLeisDeNewton}.
	\item Desloque o sensor distanciando-o mais \np[cm]{4,00} da posição inicial do carrinho. \label{MRUV-LOOP}
	\item Pressione o botão \texttt{START} novamente para zerar o cronômetro e uma segunda vez para ligar o eletroímã;
	\item Reposicione o carrinho na posição inicial; 
	\item Pressione novamente o botão \texttt{START} para liberar o carrinho novamente e anote os novos valores de tempo e de distância percorrida na Tabela~\ref{TabelaDadosLeisDeNewton}.
	\item Repita os itens acima a partir do item \ref{MRUV-LOOP}, extraindo tantos dados quanto possível. Observe que conforme o conjunto de anilhas desce, ele eventualmente colide com o chão. \textbf{A colisão com o chão só pode acontecer após o carrinho ter passado pelo segundo sensor.} Portanto, o valor máximo de distância entre os sensores estará limitado por esse fator. 
\end{enumerate}
...
\begin{question}[type={exam}]
Elabore\footnote{Na verdade, o deslocamento $\Delta x$ é a variável independente. Por questões didáticas, no entanto, estamos optando por tratar o tempo como variável independente.} um gráfico para os dados $\Delta x \times t$.
\end{question}

\begin{question}[type={exam}]
Para cada um dos pontos experimentais, determine a aceleração correspondente usando a Equação~\eqref{LeisDeNewton:Eq:AceleracaoCinematica} e preencha a seção de Dados Calculados da Tabela~\ref{TabelaDadosLeisDeNewton}. Ao final, determine o valor médio da aceleração.
\end{question}
...
\end{equation}
%
onde $a$ é o valor obtido para a aceleração através das Leis de Newton e $a_{\textrm{Ref}}$ é a média dos valores de aceleração determinados através da cinemática.
\end{question}
