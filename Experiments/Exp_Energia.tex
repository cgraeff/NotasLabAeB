...
Vamos analisar um sistema composto por um trilho sobre o qual desliza um carrinho sujeito a uma aceleração devida à tensão em uma corda que sustenta um corpo suspenso e cuja representação esquemática é dada pela Figura~\ref{Fig:MaquinaAtwoodHorizEnergia}. Nos valeremos do choque do corpo suspenso com o solo para que possamos cessar a aceleração do corpo sobre o trilho, permitindo que possamos determinar a velocidade final do sistema através da cinemática, e que possamos compará-la ao valor obtido através da conservação da energia mecânica. Verificaremos a dependência da velocidade com a altura $h$ percorrida pelo corpo suspenso, com massa $m_1$ do carrinho, e com a massa $m_2$ do corpo suspenso.
...
\begin{itemize}
	\item Trilho, carrinho, sensores, e cronômetro;
	\item Balança;
	\item Régua;
...
\begin{enumerate}
	...
	\item Disponha o carrinho, sem anilhas, sobre o trilho. O carrinho deve tocar o eletroímã, de forma que possa ser segurado por ele;
	\item Ligue um segmento do fio flexível desde o carrinho, paralelamente ao trilho, passando sobre a polia e então descendo verticalmente, até uma distância de aproximadamente \np[cm]{15,00} do solo;
...
\begin{enumerate}
    \item Ajuste o aparato conforme descrito na Seção~\ref{subsec:ajustedoaparatoenergia};
    	\item Ligue/reinicie o cronômetro e o eletroímã;
...
\begin{enumerate}
    \item Ajuste o aparato conforme descrito na Seção~\ref{subsec:ajustedoaparatoenergia};
    	\item Ligue/reinicie o cronômetro e o eletroímã;
	\item Desligue o eletroímã, liberando a movimentação do sistema. Registre o tempo medido pelo cronômetro na Tabela~\ref{TabelaDadosEnergiaMecanica1}.
	\item Tome uma anilha para o gancho e verifique sua massa. Adicione-a ao gancho e anote na Tabela~\ref{TabelaDadosEnergiaMecanica1} a nova massa \emph{total} $m_2$ suspensa.
