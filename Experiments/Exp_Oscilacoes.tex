...
\end{equation}
%
onde $k$ é uma constante de proporcionalidade cujo valor varia de mola para mola. Adotando um sistema de referência cujo eixo $x$ aponta para a direita na Figura~\ref{Fig:SentidoLeiDeHooke}, podemos escrever a expressão para a a componente da força elástica no eixo $x$ como
\begin{equation}
    F_x = -k \Delta x,
\end{equation}
%
onde o sinal negativo significa que a força é no sentido contrário à distensão da mola. Note que $x$ representa a posição do bloco, de forma que $\Delta x$ corresponde à distensão $\ell$ da mola. Se notarmos que $|F| = F_x$ e adotarmos a origem do eixo $x$ na posição inicial do bloco, temos que $x_i = 0$ e $x_f = x$, o que nos permite escrever a expressão para a força como
\begin{equation}
    F = -kx.
\end{equation}
...
\begin{tikzpicture}[>=Stealth,
    ...
    \draw[->, thick] (0, -2.4) -- +(0,1.5) node[right]{$\vec{F}_e$};
    
    \draw[->] (1,-1.0) -- (1,-3.9) node[below right]{$x$};
    \draw (0.9,-2) -- (1.1,-2) node[right]{0};
\end{tikzpicture}
\caption{Direção e sentido das forças em um sistema massa-mola.\label{Fig:MassaMolaEqDif}}
\end{marginfigure}

Vamos agora considerar um sistema onde penduramos uma mola em um suporte, de forma que ela se disponha verticalmente, e prendemos um corpo de massa $m$ à sua extremidade inferior (Figura~\ref{Fig:MassaMolaEqDif}). Soltamos o corpo a partir da posição onde a mola está relaxada, e que corresponde à coordenada $x = 0$. Utilizando a segunda lei de Newton, podemos descrever a dinâmica do corpo através de
\begin{align}
    F_R^x &= m a_x \\
	P^x + F_e^x &= ma_x \\
	P - kx &= m a_x \\
...
\end{align}
%
Definindo uma variável $x'$ tal que\footnote{A constante $mg/k$ na expressão acima representa a distensão da mola após a adição da massa $m$ em relação relação ao tamanho que ela tinha quando relaxada.}
\begin{equation}\label{eq:MassaMolaDefXPrime}
	x' = x - \frac{mg}{k},
\end{equation}
%
podemos escrever
...
verificamos que
\begin{align}
	\frac{d}{dt}x'(t) &= v'(t) = -A\omega\sen(\omega t + \phi) \\
	\frac{d^2}{dt^2} x'(t) &= a'(t) = -A\omega^2\cos(\omega t + \phi).
\end{align}
...
Verificamos então que a forma suposta para $x'(t)$ é solução da Equação~\eqref{Eq:EquacaoDiferencialOHS} se $\omega = \sqrt{k/m}$. Para determinarmos as constantes $A$ e $\phi$, precisamos usar a Equação~\eqref{eq:MassaMolaDefXPrime} para escrever $x$ em função de $x'$, obtendo
\begin{align}
	x &= x' + \frac{mg}{k} \\
	&= A \cos(\omega t + \phi) + \frac{mg}{k}.
\end{align}

Sabemos que o bloco será solto a partir da posição $x = 0$, e por isso essa será o menor valor de $x$ possível. Portanto, tal posição é um ponto de mínimo da função $x(t)$. Derivando a expressão acima e igualando a zero, obtemos
\begin{equation}
	\frac{d}{dt} x(t) = -A\sen(\omega t + \phi) = 0,
\end{equation}
%
o que implica em $\phi = 0$. Além disso, sabemos que $x(t = 0) = 0$, logo
\begin{align}
	A \cos(\omega \cdot 0) + \frac{mg}{k} &= 0\\
	A \cos (0) + \frac{mg}{k} &= 0 \\
	A + \frac{mg}{k} &= 0,
\end{align}
%
...
\begin{tikzpicture}[>=Stealth]
    ...
    \draw[smooth, samples=1000, domain=0:3.6] plot (\x, {1 - cos(3*\x r)})  node[right]{$x(t)$};
    \draw[dashed, smooth, samples=1000, domain=0:3.6] plot (\x, {sin(3*\x r)}) node[right]{$v(t)$};
    \draw[dotted, smooth, samples=1000, domain=0:3.6] plot (\x, {cos(3*\x r)}) node[right]{$a(t)$};
...
Finalmente, a expressão para $x(t)$ é
\begin{equation}
	x(t) = \frac{mg}{k} \cdot [1 - \cos(\omega t)],
\end{equation}
%
cujas derivadas são
\begin{align}
	v(t) = \frac{d}{dt} &= \frac{mg}{k} \cdot \omega\sen(\omega t)\\
	a(t) = \frac{d^2}{dt^2} &= \frac{mg}{k} \cdot \omega^2\cos(\omega t).
\end{align}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\paragraph{Interpretação de $\omega$}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Concluímos, portanto, que a posição do objeto descreve uma curva senoidal em um gráfico da posição em função do tempo. Além disso, a velocidade e a aceleração também dependem do tempo através de funções trigonométricas seno e cosseno.
\begin{marginfigure}
\centering
\begin{tikzpicture}[>=Stealth]
