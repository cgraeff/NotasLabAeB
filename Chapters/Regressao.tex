...
Determine os valores dos coeficientes linear $A$ e angular $B$, bem como os valores do coeficiente de correlação linear $R$ e de seu quadrado para os dados experimentais mostrados nas Tabelas~\ref{tab:conjuntos1a3} e~\ref{tab:conjuntos4a6}. 
\end{question}

\begin{table}[htbp]
    \centering
    \caption{Conjuntos de dados 1 a 3.}
    \label{tab:conjuntos1a3}

	\begin{tabular}{cccccccccc}
		\toprule
		& \multicolumn{2}{l}{\textbf{Conjunto 1}} & & \multicolumn{2}{l}{\textbf{Conjunto 2}} & & \multicolumn{2}{l}{\textbf{Conjunto 3}} & \\
...
\begin{table}[htbp]\forcerectofloat
    \centering
    \caption{Conjuntos de dados 1 a 3.}
    \label{tab:conjuntos4a6}

	\begin{tabular}{cccccccccc}
		\toprule
		& \multicolumn{2}{l}{\textbf{Conjunto 4}} & & \multicolumn{2}{l}{\textbf{Conjunto 5}} & & \multicolumn{2}{l}{\textbf{Conjunto 6}} & \\
    		\cmidrule{2-3} \cmidrule{5-6} \cmidrule{8-9}
        & $x$ & $y$ && $x$ & $y$ && $x$ & $y$ \\
        \cmidrule{2-3} \cmidrule{5-6} \cmidrule{8-9}
         & \np{2,50}  & \np{22,4} && \np{100,0} & \np{52,1} && \np{1,15} & \np{7,42} \\
...
\begin{question}[type={exam}]
Para cada uma das equações abaixo, considere as variáveis independentes ($X$) e dependentes ($Y$) fornecidas e as linearize. Note que algumas equações possuem mais que uma linearização possível. Também pode ser necessário que algum parâmetro seja restrito a um valor zero para que a linearização seja possível.
...
\begin{enumerate}[label=\roman*.]
    ...
    \item $X = P$, $Y = T$,
    \begin{equation}
        P = \varepsilon \sigma A
        \left(T^4-T_{\mathrm{amb}}^4\right).
    \end{equation}

    \item $X = v_t$, $Y = \theta$,
    \begin{equation}
        v_t = \sqrt{\frac{Mg}{C\rho A/2} \sen\theta}.
    \end{equation}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Respostas de alguns exercícios}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{multicols}{2} \setlength{\columnseprule}{0.4pt}
\begin{description}
	\item[ii.] $R = \frac{v_0^2}{g} \cdot \lambda$, onde $\lambda = \sen(2\theta)$. $X = \lambda = \sen(2\theta)$, $Y = R$, $A = 0$, $B = v_0^2/g$.
	\item[iv.] $T = \frac{2\pi}{\sqrt{k}} \cdot \lambda$, onde $\lambda = \sqrt{m}$. $X = \lambda$, $Y = T$, $A = 0$, $B = 2\pi/\sqrt{m}$. Alternativamente, $T^2 = \frac{4\pi^2}{k} \cdot m$, com $X = m$, $Y = T^2$, $A = 0$, $B = 4\pi/k$.
	\item[vii.] $\ln\left(1-\frac{V}{V_0}\right) = -\frac{t}{RC}$. $X = t$, $Y = \ln (1 - V/V_0)$, $A = 0$, $B = -1/(RC)$.
	\item[viii.] $P = \varepsilon\sigma A \cdot \lambda - \varepsilon\sigma A T_{\text{amb.}}^4$, onde $\lambda = T^4$. $X = \lambda = T^4$, $Y = P$, $A = - \varepsilon\sigma A T_{\text{amb.}}$, $B = \varepsilon\sigma A$.
\end{description}
\end{multicols}
